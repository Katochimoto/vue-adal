...
var AuthenticationContext = function () {
  ...
  function AuthenticationContext(opts) {
    ...
    if (opts.router) {
      ...
      opts.router.beforeEach(function (to, from, next) {
        ...
        } else if (opts.config.localAuthUrl && to.matched.some(function (record) {
  ...
  createClass(AuthenticationContext, [{
    ...
    key: 'getCachedToken',
    value: function getCachedToken(endpoint) {
      var id = endpoint && this.config.endpoints && this.config.endpoints[endpoint] || this.config.clientId;
      var authenticated = this.adalContext.getCachedToken(id);
...
var AxiosAuthHttp$$1 = function () {
  ...
  createClass(AxiosAuthHttp$$1, null, [{
    ...
    value: function createNewClient(options) {
      ...
      var token = exports.AuthenticationContext.getCachedToken(options.baseUrl);
      ...
      http.interceptors.response.use(function (response) {
        ...
        if (error.response.status === 401) {
          var retry = error.request.config.headers['X-Retry'];
          if (retry > 1) {
        ...
        if (to.matched.some(function (record) {
          ...
          getToken(options.resourceId, http, function (err, token) {
            if (err) {
              ...
              var login = exports.AuthenticationContext.config.localLoginUrl;

              if (login) {
                login = (typeof login === 'undefined' ? 'undefined' : _typeof(login)) === 'object' ? login : {
                  path: login
                };
